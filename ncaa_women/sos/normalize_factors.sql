...
from ncaa_women._parameter_levels
where parameter='year'
);

drop table if exists ncaa_women._factors;

create table ncaa_women._factors (
       parameter		text,
       level			text,
       type			text,
       method			text,
...
insert into ncaa_women._factors
...
parameter,
avg(raw_factor)
from ncaa_women._factors
where parameter not in ('o_div','d_div')
group by parameter
);

update ncaa_women._factors
set raw_factor=raw_factor-s.mean
from scale s
where s.parameter=ncaa_women._factors.parameter;
*/
