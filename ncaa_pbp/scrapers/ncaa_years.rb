...
ncaa_years = CSV.open("tsv/ncaa_years.tsv","w",{:col_sep => "\t"})
ncaa_years_divisions = CSV.open("tsv/ncaa_years_divisions.tsv","w",{:col_sep => "\t"})
...
years.each do |year|
  ...
  page2.parser.xpath(division_path).each do |child|
    ...
    ncaa_teams = CSV.open("tsv/ncaa_teams_#{year}_#{division}.tsv","w",{:col_sep => "\t"})
